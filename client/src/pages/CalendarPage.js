...
export default function CalendarPage() {
  ...
  const addTask = async () => {
    ...
    try {
      const r = await api.post('/tasks',{title:form.title,due_date:form.dueDate,priority:form.priority,notes:form.notes});
      setTasks(t=>[r.data,...t]);
      setShowAdd(false);
  ...
  return (
    <div className="page-inner">
      <PageHeader title="Calendar &amp; Tasks" sub={monthName}>
        <button className="btn btn-teal" onClick={()=>setShowAdd(true)}>+ Add Task</button>
      </PageHeader>
          ...
          <div style={{padding:20}}>
            <div className="cal-grid">
              ...
              {Array.from({length:daysInMonth},(_,i)=>i+1).map(d=>(
                <div key={d} className={'cal-day'+(d===now.getDate()?' today':'')+(taskDays.has(d)&&d!==now.getDate()?' has-event':'')}>{d}</div>
              ))}
            </div>
          </div>
        </div>
            ...
            <div>
              {loading ? (
                ...
                <div key={t.id} className="task-item" onClick={()=>toggleTask(t)}>
                  ...
                  {t.due_date&&(<span className="task-due">{new Date(t.due_date).getDate()}/{new Date(t.due_date).getMonth()+1}</span>)}
                </div>
              ))}
            </div>
          </div>
      ...
      {showAdd&&(
        <Modal title="Add Task" onClose={()=>setShowAdd(false)} footer={
          ...
          <div className="form-group">
            ...
            <input className="form-input" value={form.title} onChange={e=>setForm(f=>({...f,title:e.target.value}))} placeholder="Renew passport" autoFocus/>
          </div>
