  ...
  return (
    <div className="page-inner">
        ...
        <div className="audit-ring-wrap">
          <div style={{ textAlign: 'center', flexShrink: 0 }}>
            <svg width="130" height="130" viewBox="0 0 130 130">
              ...
              <circle cx="65" cy="65" r="52" fill="none" stroke={scoreColor(familyScore)} strokeWidth="10"
                ...
                transform="rotate(-90 65 65)" style={{ transition: 'stroke-dashoffset .8s' }} />
              <text x="65" y="62" textAnchor="middle" fontFamily="DM Serif Display,serif" fontSize="28" fill="#111827">{familyScore}</text>
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--txt2)', marginBottom: 16 }}>
              {totalDocs} documents across {audit.length} members.
              {totalMissing > 0 ? ' ' + totalMissing + ' missing categories identified.' : ' All critical categories are covered.'}
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
              {[[totalDocs, 'Filed', 'teal'], [totalMissing, 'Missing', 'amber'], [audit.filter(m => m.score < 60).length, 'At Risk', 'red'], [audit.filter(m => m.score >= 80).length, 'Complete', 'green']].map(([v, l, c]) => (
                <div key={l} style={{ textAlign: 'center', padding: 12, background: 'var(--' + c + '-bg)', borderRadius: 8, border: '1px solid var(--' + c + ')' }}>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px 8px', fontSize: 11, fontWeight: 700, color: 'var(--txt3)', textTransform: 'uppercase', letterSpacing: '.07em' }}>Member Document Health</div>
        {loading ? (
