...
export default function PortfolioPage() {
  ...
  useEffect(() => { api.get('/portfolio/summary').then(r => setData(r.data)).finally(() => setLoading(false)); }, []);
  useEffect(() => {
    if (!data?.snapshots?.length || !barRef.current) return;
    if (barChart.current) barChart.current.destroy();
    barChart.current = new Chart(barRef.current.getContext('2d'), {
      type:'bar',
      data:{ labels:data.snapshots.map(s=>new Date(s.snapshot_date).toLocaleDateString('en-IN',{month:'short'})), datasets:[{label:'Assets (Rs.L)',data:data.snapshots.map(s=>+(s.total_assets/100000).toFixed(0)),backgroundColor:'#0a9e9e'},{label:'Liabilities (Rs.L)',data:data.snapshots.map(s=>+(s.total_liabilities/100000).toFixed(0)),backgroundColor:'rgba(220,38,38,.55)'}] },
      options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top',labels:{font:{size:11}}}},scales:{x:{grid:{display:false},ticks:{font:{size:10},color:'#9ca3af'}},y:{ticks:{font:{size:10},color:'#9ca3af',callback:v=>'Rs.'+v+'L'}}}},
    });
  ...
  return (
    <div className="page-inner">
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
        <div className="card">
          <div className="sec-bar sec-bar-teal">Assets vs Liabilities</div>
          <div style={{padding:'12px 18px 18px',height:210}}><canvas ref={barRef}/></div>
        </div>
        <div className="card">
          <div className="sec-bar sec-bar-blue">Asset Allocation</div>
          <div style={{display:'grid',gridTemplateColumns:'auto 1fr',alignItems:'center',padding:'12px 18px 18px',gap:14}}>
      ...
      <div className="card" style={{marginBottom:14}}>
        <div className="sec-bar sec-bar-teal" style={{display:'flex',justifyContent:'space-between',alignItems:'center',paddingRight:14}}>
          <span>Assets</span>
          <button className="btn btn-sm" style={{background:'rgba(255,255,255,.15)',border:'1px solid rgba(255,255,255,.25)',color:'#fff',height:26,padding:'0 10px',fontSize:11}}>+ Add Asset</button>
        </div>
        <div style={{overflowX:'auto'}}>
          <table className="data-table">
            ...
            <tbody>
              {(data?.assets||[]).map(a => (
                <tr key={a.id}>
                  <td><div style={{fontWeight:600,color:'var(--txt)'}}>{a.name}</div><div style={{fontSize:11,color:'var(--txt4)'}}>{a.subtitle}</div></td>
                  ...
                  <td>
                    <div style={{display:'flex',alignItems:'center',gap:8}}>
                      <div className="progress-track" style={{flex:1,minWidth:55}}><div className="progress-fill" style={{width:((a.value/(data?.totalAssets||1))*100).toFixed(0)+'%',background:'var(--brand)'}}/></div>
                      <span style={{fontSize:12,color:'var(--txt3)'}}>{((a.value/(data?.totalAssets||1))*100).toFixed(0)}%</span>
                    </div>
                  </td>
                  <td><div style={{display:'flex',gap:6}}><button className="btn btn-outline btn-sm">Edit</button><button className="btn btn-outline btn-sm" style={{color:'var(--red)'}}>Del</button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="card">
        <div className="sec-bar sec-bar-orange" style={{display:'flex',justifyContent:'space-between',alignItems:'center',paddingRight:14}}>
          <span>Liabilities</span>
          <button className="btn btn-sm" style={{background:'rgba(255,255,255,.15)',border:'1px solid rgba(255,255,255,.25)',color:'#fff',height:26,padding:'0 10px',fontSize:11}}>+ Add Liability</button>
        </div>
        <div style={{overflowX:'auto'}}>
          <table className="data-table">
            ...
            <tbody>
              {(data?.liabilities||[]).map(l => (
                <tr key={l.id}>
                  <td><div style={{fontWeight:600,color:'var(--txt)'}}>{l.name}</div><div style={{fontSize:11,color:'var(--txt4)'}}>{l.subtitle}</div></td>
