...
export default function FamilyPage({ navigate }) {
  ...
  useEffect(() => { api.get('/family/members').then(r => setMembers(r.data)).finally(() => setLoading(false)); }, []);
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(210px,1fr))', gap:14 }}>
          {members.map(m => (
            <div key={m.id} className="card member-card" onClick={() => navigate('edit-member',m)}>
              <Avatar firstName={m.first_name} lastName={m.last_name} color={m.avatar_color||'var(--brand)'} size={60} />
              <div className="member-card-name">{m.first_name} {m.last_name}</div>
              <div className="member-card-sub">{m.relation||m.role}</div>
              <div className="member-card-stats">
                <div className="member-stat"><div className="member-stat-val">{m.document_count||0}</div><div className="member-stat-lbl">Docs</div></div>
                <div className="member-stat"><div className="member-stat-val">{m.net_worth?fmtK(m.net_worth):'--'}</div><div className="member-stat-lbl">Net Worth</div></div>
          ...
          <button className="card" onClick={() => navigate('add-member')} style={{ minHeight:190, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:10, cursor:'pointer', background:'var(--surface2)', border:'2px dashed var(--border2)', borderRadius:'var(--r-lg)' }}>
