...
export default function AddMemberPage({ navigate, editMember }) {
  ...
  const [form, setForm] = useState({
    ...
    avatarColor: editMember?.avatar_color || '#0f1f3d',
  });
  ...
  return (
    <div className="page-inner" style={{ maxWidth: 760 }}>
      ...
      {error && <div style={{ background: 'var(--rose-bg)', border: '1px solid var(--rose)', borderRadius: 8, padding: '12px 16px', marginBottom: 16, color: 'var(--rose)', fontSize: 13 }}>{error}</div>}

      {/* Personal details */}
      <div className="card" style={{ padding: 28, marginBottom: 16 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--txt2)', textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 18 }}>Personal Details</div>
        <div className="form-grid-2" style={{ marginBottom: 0 }}>
          {[['First Name','firstName','text','Arun'],['Last Name','lastName','text','Kumar'],['Email','email','email','email@example.com'],['Phone','phone','tel','+91 XXXXX XXXXX'],['Date of Birth','dateOfBirth','date',''],['Age / Year','','','']].slice(0,5).map(([l,k,t,ph]) => (
        ...
        <div className="form-grid-2">
          <div className="form-group">
            ...
            <select className="form-select" value={form.role} onChange={upd('role')}>
              {['admin','co-admin','member','advisor','view-only'].map(r => <option key={r} value={r} style={{ textTransform: 'capitalize' }}>{r}</option>)}
            </select>
          </div>
          <div className="form-group">
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 6 }}>
              {COLORS.map(c => (
                <button key={c} onClick={() => setForm(f => ({ ...f, avatarColor: c }))}
                  style={{ width: 28, height: 28, borderRadius: '50%', background: c, border: form.avatarColor === c ? '3px solid var(--teal)' : '2px solid transparent', cursor: 'pointer', outline: form.avatarColor === c ? '2px solid rgba(7,185,138,.4)' : 'none' }} />
              ))}
            </div>
          </div>
        </div>
      ...
      {/* Permissions */}
      <div className="card" style={{ padding: 28, marginBottom: 16 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--txt2)', textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 18 }}>Access Permissions</div>
        {[['Document Vault','vault',['full','view','own','none']],['Portfolio','portfolio',['full','view','none']],['AI Insights','insights',['full','view','none']],['Family Mgmt','family_mgmt',['admin','co-admin','none']]].map(([l,k,opts]) => (
          <div key={k} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid var(--border)' }}>
            ...
            <select className="form-select" value={perms[k]} onChange={updP(k)} style={{ width: 140, height: 34 }}>
      ...
      <div className="card" style={{ padding: 24 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--txt2)', textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 10 }}>
          {isEdit ? 'Save Changes' : 'Invitation'}
        </div>
        <p style={{ fontSize: 13, color: 'var(--txt2)', marginBottom: 16 }}>
          {isEdit
            ? 'Changes will take effect immediately. The member will see updated permissions on their next login.'
            : 'An email invitation will be sent to the address above. They will set their own password. Temporary password: familyos123'}
        </p>
