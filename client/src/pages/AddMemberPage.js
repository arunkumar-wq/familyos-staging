...
export default function AddMemberPage({ navigate, editMember }) {
  ...
  return (
    <div className="page-inner" style={{maxWidth:760}}>
      ...
      {error&&(<div style={{background:'var(--red-bg)',border:'1px solid var(--red-border)',borderRadius:8,padding:'10px 14px',marginBottom:16,color:'var(--red)',fontSize:13}}>{error}</div>)}
      <div className="card" style={{marginBottom:14}}>
        <div className="sec-bar sec-bar-teal">Personal Details</div>
        <div style={{padding:20}}>
          <div className="form-grid-2">
            {[['First Name','firstName','text','Arun'],['Last Name','lastName','text','Kumar'],['Email','email','email','email@example.com'],['Phone','phone','tel','+91 XXXXX XXXXX'],['Date of Birth','dateOfBirth','date','']].map(([l,k,t,ph]) => (
            ...
            <div className="form-group">
              ...
              <div style={{display:'flex',gap:6,flexWrap:'wrap',marginTop:6}}>
                {COLORS.map(c=>(<button key={c} onClick={()=>setForm(f=>({...f,avatarColor:c}))} style={{width:26,height:26,borderRadius:'50%',background:c,cursor:'pointer',border:form.avatarColor===c?'3px solid var(--txt)':'2px solid transparent',outline:'none'}}/>))}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="card" style={{padding:20}}>
        <p style={{fontSize:13,color:'var(--txt2)',marginBottom:16}}>
          {isEdit?'Changes will take effect immediately.':'An email invitation will be sent. Temporary password: familyos123'}
        </p>
